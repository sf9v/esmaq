...
func Gen(cfg Schema, out io.Writer) error {
	...
	ctxIDC := jen.Id("ctx")
	ctxVarC := jen.Id("ctx").Qual("context", "Context")
	ctxQualC := jen.Qual("context", "Context")
	stateQualC := jen.Qual(pkgPath, "State")

	f.Func().Id("CtxWtFrom").
		Params(ctxVarC,
			jen.Id("from").Qual(pkgPath, "State"),
		).
		Params(ctxQualC).
		BlockFunc(func(g *jen.Group) {
			g.Return(jen.Qual("context", "WithValue").
				Call(ctxIDC, jen.Id("fromKey"), jen.Id("from")))
		}).Line()

	f.Func().Id("ctxWtTo").
		Params(ctxVarC,
			jen.Id("to").Qual(pkgPath, "State"),
		).
		Params(ctxQualC).
		BlockFunc(func(g *jen.Group) {
			g.Return(jen.Qual("context", "WithValue").
				Call(ctxIDC, jen.Id("toKey"), jen.Id("to")))
		}).Line()

	f.Func().Id("fromCtx").
		Params(ctxVarC).
		Params(stateQualC, jen.Bool()).
		BlockFunc(func(g *jen.Group) {
			...
			methods = append(methods, jen.Func().
				...
				BlockFunc(func(g *jen.Group) {
					...
					g.Id("err").Op("=").Id(rcvr).
						Dot("core").
						Dot("Fire").
						Call(jen.Id(strcase.ToCamel("event_"+string(trsn.Event))),
							jen.Id("from"))
